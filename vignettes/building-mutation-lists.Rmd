...
1. Using a mutation list curated by the PANGO team ("Constellations")
    - These are (mostly) representative mutations - there is very little overlap between the mutation lists for different variants.
    - The team takes great pains to keep them updated and useful.
    - This list only a specific subset of variants of concern with a small subset of their mutations and may not be useful for all purposes.
2. By specifying the variants that are of interest to you and then pruning down the NextStrain GenBank data.
3. Using variants that contain the same mutations as the ones in your data.
...
From within R, you can get the variant matrix using the following code:
...
ProVoC also contains a function to build mutation lists based on a list of variant names.
...
The function `varmat_by_variants()` allows for a specification of the sample date.
...
The function also incorporates the date that the database  to ensure that currently circulating variants are not assumed to be missing simply because they haven't been sequenced recently.
